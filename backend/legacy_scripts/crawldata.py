...
class Crawl:
    ...
    def get_info(self):
        ...
        try:
            ...
            if 'errors' in data:
                error_msg = data['errors'][0].get('message', 'API返回错误') if data['errors'] else 'API返回错误'
                raise Exception(f'Cookie无效或已过期: {error_msg}')
            
            user = data.get('data', {}).get('userAuth')
            if not user:
                raise Exception('获取用户信息失败，请重新扫码')
            
            oftenseat = user.get('oftenseat', {}).get('list', [])
            if len(oftenseat) < 1:
                raise Exception('未设置常用座位，请先在公众号设置')
            
            seat_info = {
                'lib_id': oftenseat[0]['lib_id'], 
                'seat_key': oftenseat[0]['seat_key'],
                'info': oftenseat[0]['info']
            }
            return seat_info
